...
| Catalog discovery, metadata, sale/test axes | `domain/catalog`, discovery/test workers, `quest_test_batches` | Monitor-gate integration test: 3 attempts per Monitor, first pass stops, failures create private override, and a Quest Manual Review can retry/reseed a batch; customer-account admission and customer `quest-new` announcement test | live Quest API metadata drift and Monitor test UAT |
...
| Surface setup permissions | Discord startup/surface privacy/outbox workers, migration `0020_remove_surface_expected_permissions.sql` | startup refuses a Bot without Administrator; no persisted drift snapshot; private `LOG_PAYMENTS` visibility is checked before decrypt/render and unsafe events stay in Financial DLQ | production channel-layout setup test |
...
| Backup, restore, retention, keys | S3 backup stream, drill script, retention/key worker | fake S3/pg_dump upload-verify-decrypt contract with versioned manifest, temporary CA hand-off/cleanup for `pg_dump` and `pg_restore`, dump termination after upload failure, confirmed checkout-session/credential cleanup and 500-row `SKIP LOCKED` retention batch, stream code checks and ledger retention test | real S3 backup and temporary-DB restore drill |
| Deployment, rollback, CI, load | Dockerfile, workflow, scripts, UAT docs | CI Docker build plus disposable 200-user/100-order load test | same-SHA deploy/rollback rehearsal |
