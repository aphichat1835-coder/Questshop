...
|---|---|---|---|
| Node 22 ESM, config, PostgreSQL pools/TLS/time | `src/config`, `src/db`, migrations 0001–0012 | syntax/lint/migration checksum tests | managed PostgreSQL role and CA validation |
| State machines, CAS, correlation, audit | domain `states.js`, `state_transitions`, domain services | state/unit and integration tests | production trace sampling |
...
| TrueMoney Direct, receiver snapshot, HMAC, ambiguity | `adapters/truemoney`, payment worker/services | duplicate voucher and crash-credit tests | real success/ambiguous/schema fixtures |
...
| Catalog discovery, metadata, sale/test axes | `domain/catalog`, discovery/test workers | contract/state tests | live Quest API metadata drift |
| Checkout, quote revalidation, account lock | `domain/checkout`, interaction router | large-order/account uniqueness tests | mobile Discord UAT |
...
| Outbox, coalescing, Discord errors, DLQ | outbox services/workers/renderers | DLQ replay/discard and 403 drift tests | 404/429/network Discord fault UAT |
...
| Permission drift and repair | Discord permission monitor/repair | contract checks; live mutation not simulated | production overwrite drift/repair test |
| Health, metrics, SLO alerts, gates | bootstrap health and alert worker | financial invariant alert test | external alert delivery/uptime observation |
| Backup, restore, retention, keys | S3 backup stream, drill script, retention/key worker | stream code checks and ledger retention test | real S3 backup and temporary-DB restore drill |
