...
export async function routeInteraction(interaction) {
  ...
  try {
    ...
    if (!route) return;
    if (['start','topup','admin'].includes(route.route)) {
      ...
      if (!surface || surface.guild_id!==interaction.guildId || surface.channel_id!==interaction.channelId
        || surface.message_id!==interaction.message?.id) throw new QuestshopError('SURFACE_BINDING_INVALID', 'ปุ่มนี้ไม่ใช่แผงที่ใช้งานอยู่');
    }
    ...
    const backofficeRoute = ['admin', 'gate_', 'wallet_', 'refund_', 'block_', 'review_', 'perm_',
      'price_', 'promo_', 'receiver_', 'monitor_', 'catalog_', 'adminorder_', 'dlq_', 'config_', 'breaker_']
      .some((prefix) => route.route === prefix || route.route.startsWith(prefix));
    if (backofficeRoute && !isBackoffice(interaction, runtime)) {
      throw new QuestshopError('ADMIN_ONLY', 'เมนูนี้ใช้ได้เฉพาะ Owner/Admin');
    }
    if (!backofficeRoute && (!gates.STORE_OPEN || !gates.CUSTOMER_INTERACTIONS_ENABLED)) {
      throw new QuestshopError('STORE_CLOSED', 'ร้านปิดรับรายการชั่วคราว');
    }
    if (['payment_method','voucher_submit'].includes(route.route) && !gates.TOPUP_ACCEPTING) {
      throw new QuestshopError('TOPUP_CLOSED', 'ระบบเติมเงินปิดชั่วคราว');
    }
    if (['token_submit','quest_confirm'].includes(route.route) && !gates.ORDER_ACCEPTING) {
      throw new QuestshopError('ORDER_CLOSED', 'ระบบรับ Quest ปิดชั่วคราว');
    }
    if (route.route === 'start' && interaction.isButton()) {
      if (!gates.ORDER_ACCEPTING) throw new QuestshopError('ORDER_CLOSED', 'ระบบรับ Quest ปิดชั่วคราว');
    ...
    if (route.route === 'topup' && interaction.isButton()) {
      if (!gates.TOPUP_ACCEPTING) throw new QuestshopError('TOPUP_CLOSED', 'ระบบเติมเงินปิดชั่วคราว');
      const entry = await createAdminSession({ actorId: interaction.user.id, guildId: interaction.guildId,
